...
if iptables -C "$DOCKER_USER_CHAIN" -s "$ip" -p tcp -m conntrack --ctdir ORIGINAL --ctorigdstport "$WEB_PORT" -m comment --comment "$RULE_COMMENT" -j DROP 2>/dev/null; then
    echo "Already blocked from web port $WEB_PORT: $ip"
    exit 0
fi

iptables -I "$DOCKER_USER_CHAIN" 1 -s "$ip" -p tcp -m conntrack --ctdir ORIGINAL --ctorigdstport "$WEB_PORT" -m comment --comment "$RULE_COMMENT" -j DROP
